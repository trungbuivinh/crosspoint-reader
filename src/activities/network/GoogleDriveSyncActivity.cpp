...
void GoogleDriveSyncActivity::releaseTreeStorageBeforeListing() {
  ...
  // handshake; clear() alone preserves capacity and can starve TLS.
...
bool GoogleDriveSyncActivity::scanLocalTree() {
  ...
  // Reserve the bounded local snapshot here so its backing block never
  // competes with a TLS handshake.
