...
TEST(OtaVersion, CustomRevisionUpdateIsNewer) {
  EXPECT_EQ(OtaVersion::compare("2.3.4.0", "2.3.4.1"), Comparison::Newer);
}

TEST(OtaVersion, NewMinorWithZeroCustomRevisionIsNewer) {
  EXPECT_EQ(OtaVersion::compare("1.4.1.0", "1.5.0.0"), Comparison::Newer);
}
...
TEST(OtaVersion, StableReleaseSupersedesMatchingReleaseCandidate) {
  EXPECT_EQ(OtaVersion::compare("1.5.0-rc+abc123", "1.5.0"), Comparison::Newer);
  EXPECT_EQ(OtaVersion::compare("1.5.0.1-rc2", "1.5.0.1"), Comparison::Newer);
}

TEST(OtaVersion, CurrentBuildSuffixIsAccepted) {
  EXPECT_EQ(OtaVersion::compare("1.5.0.0-dev-release/1.5.0.0-abc123", "1.5.0"), Comparison::Equal);
  EXPECT_EQ(OtaVersion::compare("1.5.0.0+local.123", "1.5.0.1"), Comparison::Newer);
}

TEST(OtaVersion, MalformedCandidateIsRejected) {
  constexpr const char* malformedVersions[] = {"",       "1.2",      "1.2.3.4.5",      "v1.2.3",  "1.2.x",
                                               "1..2.3", "1.2.3-rc", "4294967296.0.0", "1.2.3.4."};
